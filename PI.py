...
# ============================
class _FakeRemainderColsList(list):
    """Classe fake para simular o objeto interno removido no scikit-learn mais novo."""
    pass

if not hasattr(_column_transformer, "_RemainderColsList"):
    _column_transformer._RemainderColsList = _FakeRemainderColsList


st.markdown("---")
st.header("Previsão de Tipo de Crime por Local e Horário")

# Carregar modelo treinado
bundle = joblib.load("modelo1_classificacao_crime.pkl")
model = bundle["model"]
...
with col1:
    ano = st.number_input("Ano", min_value=2015, max_value=2030, value=2025)
...
if st.button("Prever Tipo de Crime"):
    ...
    tipo_previsto = encoder.inverse_transform(pred)[0]
    st.success(f"Tipo de crime mais provável: **{tipo_previsto}**")

    # (Opcional) probabilidades detalhadas
    probs = model.predict_proba(Xq)[0]
    classes = encoder.classes_
    df_probs = pd.DataFrame({"Tipo de Crime": classes, "Probabilidade": probs})
    df_probs = df_probs.sort_values("Probabilidade", ascending=False)
    st.bar_chart(df_probs.set_index("Tipo de Crime"))
